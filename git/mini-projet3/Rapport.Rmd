...
```{r}
afd_model <- MASS::lda(Statuts ~ tfidf_Description + tfidf_Tittle, data = little_df)
```
...
# Utilisez la matrice TF-IDF directement
tfidf_matrix <- as.matrix(tfidfs$Description)

# Spécifiez le nombre de sujets (topics) que vous souhaitez extraire
num_topics <- 5  # Vous pouvez ajuster ce nombre en fonction de vos besoins

# Appliquez le modèle LDA
lda_model <- LDA(tfidf_matrix, k = num_topics, control = list(seed = 1234))

# Affichez les termes les plus représentatifs pour chaque sujet
terms(lda_model)

# Affichez la distribution des sujets pour chaque document
...
# Nouvelles caractéristiques après LDA
...
```{r}
#afd_model <- MASS::lda( Domain ~ tfidf_Description + tfidf_Tittle + Author + Direction + Statuts, data = little_df)
# afd_model <- MASS::lda( Domain ~ tfidf_Description + tfidf_Tittle, data = little_df)
```
...
# plot(projected_data[, 1], projected_data[, 2], col = as.factor(little_df$Domain),
#      pch = 19, main = "Visualisation des résultats de l'AFD", xlab = "LD1", ylab = "LD2")

# # Légende
# legend("topright", legend = levels(as.factor(little_df$Domain)), col = 1:3, pch = 19, title = "Domain")

# # Évaluation du modèle (précision dans ce cas)
# predicted_labels <- predict(afd_model, newdata = little_df)$class
# accuracy <- sum(predicted_labels == little_df$Domain) / length(little_df$Domain)
