...
```{r}
# On efface les variables en mémoires
rm(list=ls())
...
trainX <- subset(df, select = c('Salaire',	'Impots',	'Etudiant'))
...
```{r, echo = F}
trainy <- subset(df, select = c('Controle'))
...
trainX$Salaire <- as.factor(trainX$Salaire)
trainX$Salaire <- as.factor(trainX$Salaire)
trainX$Salaire <- as.factor(trainX$Salaire)
contingence <- 0
view(contingence)
